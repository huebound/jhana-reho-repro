...
from jhana_repro.classify import ClassificationConfig, run_benchmark, run_modes
...
def test_public_benchmark_writes_summary(tmp_path):
    rows = []
    for index in range(12):
        label = "depr" if index < 6 else "control"
        signal = 1.0 if label == "depr" else -1.0
    ...
    frames = run_benchmark(
        ...
        tmp_path / "benchmark",
        seeds=2,
        config=ClassificationConfig(n_splits=2, compat_target_per_subject=2),
    )

    assert set(frames) == {"by_seed", "summary", "differences", "difference_summary"}
